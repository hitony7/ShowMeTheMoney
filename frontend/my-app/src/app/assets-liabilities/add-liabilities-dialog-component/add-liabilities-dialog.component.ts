...
@Component({
  ...
  selector: 'app-add-liabilities-dialog',
  templateUrl: './add-liabilities-dialog.component.html',
  styleUrls: ['./add-liabilities-dialog.component.css']
})
export class AddLiabilitiesDialogComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<AddLiabilitiesDialogComponent>,
  ...
  save(): void {
    ...
    // Make HTTP POST request to save the liability 
    this.http.post(`http://localhost:3000/liability/newEntry`, formData)
      .subscribe({
        next: (response) => {
          console.log('liability saved successfully:', response);
          // Handle success response
        },
        error: (error) => {
          console.error('Error saving liability:', error);
          // Handle error saving liability
        }
      });
